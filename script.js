...
document.querySelectorAll('a, button, .magnetic-btn, .product-card').forEach(el => {
...
// ✨ 5. Premium Loader & Page Transition - Extreme Fading added
window.addEventListener('load', () => {
    const splitTitle = new SplitType('.split-target', { types: 'chars' });
    const splitBrand = new SplitType('.split-brand', { types: 'chars' }); // Brand entry text split
    
    const tl = gsap.timeline();
    let count = { val: 0 };
    
    // ✨ Brand Entry Animation Setup (Letter tracking and blur reveal)
    ...
    // ✨ Extreme 3D Fading Animation for Hero Text
